<script>
    $(document).ready(function(){
        $('#tb-statistik .link').click(function(){
            ...
            var viddikfung = $(par).find('.ed1').html();
            ...
            $('#form-print #iddikfung').val(viddikfung);
...
<form id="form-print" name="form-print" action="<?=base_url()?>diengplateu/page/statistik_statdikfung_print" class="form-horizontal" method="post" enctype="multipart/form-data" target="_blank">
    <input type="hidden" id="idwhere" name="idwhere" value="" />
    <input type="hidden" id="idjenkel" name="idjenkel" value="" />
    <input type="hidden" id="iddikfung" name="iddikfung" value="" />
        ...
        <table class="table table-hover table-bordered" width="100%" id="tb-statistik">
            ...
            <?php
                ...
                $rs = $this->db->query("SELECT a.dikfung AS kategori, a.iddikfung, SUM(IF(c.idjenkel = 1,1,0)) AS jmlpria, SUM(IF(c.idjenkel = 2,1,0)) AS jmlwanita FROM a_dikfung a
                        ...
                        INNER JOIN tb_1223 c ON b.nip = c.nip WHERE c.idjenkedudupeg NOT IN (99,21) and c.idstspeg='3' $where GROUP BY c.idjenkel");
                ...
                <tr>
                    <td>
                        <?php echo $x?>
                        <span class="ed1" style="display:none"><?=$item->iddikfung?></span>
                    </td>
                ...
                <tr class="breadcrumb">
                    ...
                    <td><div align="right" data="1"><?php echo number_format(array_sum($sumallp))?></div></td>
                    <td><div align="right" data="1"><?php echo number_format(array_sum($sumallw))?></div></td>
                    <td><div align="right" data="1"><?php echo number_format(array_sum($jmlall))?></div></td>
                </tr>
            </tbody>
        </table>
    </div>
