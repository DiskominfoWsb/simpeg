...
//class PDF_Code128 
    ...
    if($rs){
        ...
        // Title
        $pdf->Cell(200, 7, 'DAFTAR KEBUTUHAN JABATAN', 0, 1, 'C');
        $pdf->Cell(200, 7, strtoupper(getSkpd($idskpd)), 0, 1, 'C');
        $pdf->Cell(200, 7, 'PEMERINTAH KABUPATEN KENDAL', 0, 1, 'C');
        $pdf->Cell(200, 7, 'PERIODE '.strtoupper(formatBulan($periode_bulan)).' '.$periode_tahun, 0, 1, 'C');
        $pdf->Cell(200, 7, '', 0, 1, 'C');

        $pdf->SetFont('Arial', '', 6);

        // $pdf->Cell(10, 10, 'NO', 1, 0, 'C');
        // $pdf->Cell(30, 10, 'JENIS JABATAN', 1, 0,'C');
        // $pdf->Cell(50, 10, 'NAMA JABATAN', 1, 0, 'C');
        // $pdf->Cell(70, 5, 'SYARAT JABATAN', 1, 0, 'C');
        // $pdf->Cell(15, 10, 'KELAS', 1, 0, 'C');
        // $pdf->Cell(15, 10, 'KEBUTUHAN', 1, 0, 'C');

        $pdf->Cell(10, 10, 'NO', 1, 0, 'C');
        $pdf->Cell(70, 10, 'NAMA JABATAN', 1, 0, 'C');
        $pdf->Cell(50, 10, 'NAMA UNIT KERJA', 1, 0,'C'); //sub satuan kerja
        $pdf->Cell(30, 10, 'KUALIFIKASI PENDIDIKAN', 1, 0, 'C');
        $pdf->Cell(7, 10, 'ABK', 1, 0, 'C');
        $pdf->Cell(7, 10, 'RIIL', 1, 0, 'C');
        ...
        $no = 0;
        $jml_keb = 0;
        $pdf->SetWidths(array(10,70,50,30,7,7,10,7));
        ...
        //         $no,
        //         $item->jenjabatan,
        //         (KebutuhanjabatanModel::lengthspace($item->idskpd)." ".(($item->idjenjab < 20)?'     ':'').$item->jab),
        //         (($item->syarat_idjenjurusan!='')? KebutuhanjabatanModel::getJenisjurusan($item->syarat_idjenjurusan):'-'),
        //         (($item->syarat_iddiklat!='')? KebutuhanjabatanModel::getSyaratdiklatpdf($item->syarat_iddiklat,$item->idjenjab):'-'),
        ...
        foreach($rs as $item){
            ...
            $pdf->Rowmini(array(
                $no,
                (KebutuhanjabatanModel::lengthspace($item->idskpd)." ".(($item->idjenjab < 20)?'     ':'').$item->jab),
                (($item->subskpd!='')? $item->subskpd:'-'),
                (($item->syarat_idjenjurusan!='')? KebutuhanjabatanModel::getJenisjurusan($item->syarat_idjenjurusan):'-'),
                (($item->abk!='')?$item->abk:'-'),
                ($banyakorang),
                ($status_selisih),
                ($banyakorang-$item->abk)
            ));
        }

        // ----------------------------
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(184, 8, 'Total Kebutuhan', 1, 0, 'R');
        $pdf->Cell(7, 8, $total, 1, 0, 'C');
        // ----------------------------
        $pdf->Cell(5, 4, '', 0, 1);
    ...
    $pdf->Output('Formulir Perhitungan Kebutuhan ASN Peta Jabatan '.$idskpd.' '.formatBulan($periode_bulan).' '.$periode_tahun.'.pdf', 'I');
?>