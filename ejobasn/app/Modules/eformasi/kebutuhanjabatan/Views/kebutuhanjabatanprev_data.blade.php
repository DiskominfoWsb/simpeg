<?php
    /*cek sudah ada kebutuhanjabatan ditahun yang sama atau belum*/
    $x = 0;
...
@else
    {!! Form::open(array('url' => url('').'/eformasi/kebutuhanjabatan/kebutuhanjabatanprev', 'method' => 'POST', 'class'=>'form-horizontal form-'.\Config::get('claravel::ajax'),'id'=>'form-kebutuhanjabatanprev')) !!}
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    {!!KebutuhanjabatanModel::length($item->idskpd)." ".$item->jab!!}
                </td>
                <td>{!!($item->idtkpendid!='')? KebutuhanjabatanModel::getJenisPendidikan($item->idtkpendid):'-'!!}</td>
                <td>{!!KebutuhanjabatanModel::getJenisjurusan($item->idrumpunpendid,$item->idjenjurusan)!!}</td>
                <td>
                    <input class="form-control" align="right" type="text" name="{!!$x!!}[abk]" value="1">
                </td>
            </tr>
            @endforeach
        </tbody>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#form-kebutuhanjabatanprev').on('submit',function(e){
            var $this = $(this);
            e.preventDefault();
