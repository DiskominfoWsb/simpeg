...
//        $typePatt = strtolower(text::clearWhitespaces($this->types[$this->type]));
        $typePatt = strtolower(clearWhitespaces($this->types[$this->type]));
        ...
        elseif (preg_match('/^\*([^ ]+)(.*)?$/s', $typePatt, $patt)) {
            ...
            if (class_exists("type_$type")) {
                ...
                return $this->label("Non-existing directory type."."type_$type");
        }
...
//        $exts = strtolower(text::clearWhitespaces($this->config['deniedExts']));
        $exts = strtolower(clearWhitespaces($this->config['deniedExts']));
        if (strlen($exts)) {
            $exts = explode(" ", $exts);
