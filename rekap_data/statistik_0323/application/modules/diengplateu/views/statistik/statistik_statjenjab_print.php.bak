...
<html>
<head>
    <title>Simpeg Tegal Statistik Jenis Jabatan</title>
...
<?php
    ...
    $idjenjab = $this->input->post('idjenjab');
    $idskpd = $this->input->post('idskpd');

    $where = " where a.idjenkedudupeg not in('99','21') ";
    if($idwhere != "1") {
        $where.=" and a.idjenjab = \"".$idjenjab."\"";
        $titile_jenjab = " JENIS JABATAN ".$this->siemodel->getAttrdata("a_jenjab","idjenjab",$idjenjab,"jenjab");
    }else{
        $titile_jenjab = " SEMUA JENIS JABATAN ";
    }
...
<center>
    ...
    <h3>BERDASARKAN <?php echo $titile_kelamin?> <?php echo strtoupper($titile_jenjab)?></h3>
</center><br>
<table border="1">
    <colgroup>
