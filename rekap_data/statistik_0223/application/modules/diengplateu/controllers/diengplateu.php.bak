<?php if(! defined('BASEPATH')) exit('No direct script acess allowed.');
	class Diengplateu extends MX_Controller {
        ...
        function graphKelamin(){
            ...
            $rs = $this->db->query("
                SELECT IF(idjenkel=1,'Pria',IF(idjenkel=2,'Wanita','-')) AS kategori, COUNT(*) AS pns FROM tb_0223
        ...
        function graphDikstru(){
            ...
            $rs = $this->db->query("SELECT a.dikstru as kategori, SUM(IF(b.iddikstru!='' AND b.idjenkel = 1,1,0)) AS jmlpria, SUM(IF(b.iddikstru!='' AND b.idjenkel = 2,1,0)) AS jmlwanita FROM a_dikstru a
              inner JOIN tb_0223 b ON a.iddikstru = b.iddikstru AND b.idjenkedudupeg NOT IN (99,21) $where GROUP BY a.iddikstru");
            $delimiter = ",";
            $newline = "\n";
            $categories = "";
            foreach ($rs->list_fields() as $field){
        ...
        function graphJabfung(){
            ...
            $rs = $this->db->query("SELECT a.jabfung as kategori, SUM(IF(b.idjabfung!='' AND b.idjenkel = 1,1,0)) AS jmlpria, SUM(IF(b.idjabfung!='' AND b.idjenkel = 2,1,0)) AS jmlwanita FROM a_jabfung a
              LEFT JOIN tb_0223 b ON a.idjabfung = b.idjabfung AND b.idjenkedudupeg NOT IN (99,21) and b.idjenjab = '2' $where GROUP BY a.jabfung ORDER BY a.jabfung");
            $delimiter = ",";
            $newline = "\n";
            $categories = "";
            foreach ($rs->list_fields() as $field){
        ...
        function graphJabfungum(){
            ...
            $rs = $this->db->query("SELECT a.jabfungum as kategori, SUM(IF(b.idjabfungum!='' AND b.idjenkel = 1,1,0)) AS jmlpria, SUM(IF(b.idjabfungum!='' AND b.idjenkel = 2,1,0)) AS jmlwanita FROM a_jabfungum a
              LEFT JOIN tb_0223 b ON a.idjabfungum = b.idjabfungum AND b.idjenkedudupeg NOT IN (99,21) and b.idjenjab = '3' $where GROUP BY a.jabfungum ORDER BY a.jabfungum");
            $delimiter = ",";
            $newline = "\n";
            $categories = "";
            foreach ($rs->list_fields() as $field){
        ...
        function graphSkpd(){
            ...
            $rs = $this->db->query("SELECT replace(b.skpd,',',' ') as kategori, SUM(IF(a.idskpd!='' AND a.idjenkel = 1,1,0)) AS jmlpria, SUM(IF(a.idskpd!='' AND a.idjenkel = 2,1,0)) AS jmlwanita FROM tb_0223 a
        ...
        function graphMarital(){
            ...
            $rs = $this->db->query("SELECT a.stskawin AS kategori, SUM(IF(b.idstskawin!='' AND b.idjenkel = 1,1,0)) AS jmlpria, SUM(IF(b.idstskawin!='' AND b.idjenkel = 2,1,0)) AS jmlwanita FROM a_stskawin a
              LEFT JOIN tb_0223 b ON a.idstskawin = b.idstskawin AND b.idjenkedudupeg NOT IN (99,21) $where GROUP BY a.idstskawin");
            $delimiter = ",";
            $newline = "\n";
            $categories = "";
            foreach ($rs->list_fields() as $field){
        ...
        function graphKartu(){
            ...
            $rs = $this->db->query("SELECT a.idjenkel, SUM(IF(b.nokarpeg!='',1,0)) AS nokarpeg, SUM(IF(b.nokarpeg='',1,0)) AS nonnokarpeg,
              ...
              FROM a_jenkel a LEFT JOIN tb_0223 b ON a.idjenkel = b.idjenkel $where GROUP BY a.idjenkel ORDER BY a.idjenkel");
            $delimiter = ",";
            $newline = "\n";
            $categories = "";
            foreach ($rs->list_fields() as $field){
        ...
        function graphSekdes(){
            ...
            if($idskpd != "") $where = ($idskpd == "")?"":"AND idskpd LIKE '$idskpd%'";
            $rs = $this->db->query("
                SELECT IF(a.idjenkel=1,'Pria',IF(a.idjenkel=2,'Wanita','-')) AS kategori, COUNT(*) AS pns FROM tb_0223 a
        ...
        function graphDikfung(){
            ...
            $rs = $this->db->query("SELECT a.dikfung AS kategori, SUM(IF(c.idjenkel = 1,1,0)) AS jmlpria, SUM(IF(c.idjenkel = 2,1,0)) AS jmlwanita FROM a_dikfung a
                    ...
                    INNER JOIN tb_0223 c ON b.nip = c.nip WHERE c.idjenkedudupeg NOT IN (99,21) $where GROUP BY c.idjenkel");
            $delimiter = ",";
            $newline = "\n";
            $categories = "";
            foreach ($rs->list_fields() as $field){
