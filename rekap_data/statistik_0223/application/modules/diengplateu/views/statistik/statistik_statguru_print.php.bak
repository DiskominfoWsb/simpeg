...
<?php
    ...
    $isguru = $this->input->post('isguru');
    $idskpd = $this->input->post('idskpd');

    $where = " where a.idjenkedudupeg not in('99','21') ";
    if($isguru == "4") {
        $where.=" and MID(a.idjabfung, 5, 1) = \"".$isguru."\"";
        $titile_guru = ($isguru==4)?" STATUS GURU":" STATUS NON GURU";
    }else if($isguru == "0") {
        $where.=" and MID(a.idjabfung, 5, 1) != \"".$isguru."\"";
        $titile_guru = ($isguru==4)?" STATUS GURU":" STATUS NON GURU";
    }else{
        $titile_guru = " STATUS GURU DAN NON GURU ";
    }
...
<center>
    ...
    <h3>BERDASARKAN <?php echo $titile_kelamin?> <?php echo strtoupper($titile_guru)?></h3>
</center><br>
<table border="1">
    <colgroup>
