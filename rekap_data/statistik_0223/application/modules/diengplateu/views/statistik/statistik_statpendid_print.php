...
<html>
<head>
    <title>Simpeg Tegal Statistik Pendidikan</title>
...
<?php
    ...
    $idtkpendid = $this->input->post('idtkpendid');
    $idskpd = $this->input->post('idskpd');

    $where = " where a.idjenkedudupeg not in('99','21') ";
    if($idwhere != "1") {
        $where.=" and a.idtkpendid = \"".$idtkpendid."\"";
        $titile_pendidikan = " PENDIDIKAN ".$this->siemodel->getAttrdata("a_tkpendid","idtkpendid",$idtkpendid,"tkpendid");
    }else{
        $titile_pendidikan = " SEMUA PENDIDIKAN ";
    }
...
<center>
    ...
    <h3>BERDASARKAN <?php echo $titile_kelamin?> <?php echo strtoupper($titile_pendidikan)?></h3>
</center><br>
<table border="1">
    <colgroup>
