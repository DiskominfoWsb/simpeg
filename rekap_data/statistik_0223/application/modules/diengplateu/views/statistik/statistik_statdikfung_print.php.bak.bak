...
<html>
<head>
    <title>Simpeg Tegal Statistik Diklat Fungsional</title>
...
<?php
    ...
    $iddikfung = $this->input->post('iddikfung');
    $idskpd = $this->input->post('idskpd');

    $where = " where a.idjenkedudupeg not in('99','21') ";
    if($idwhere != "1") {
        $where.=" and k.iddikfung = \"".$iddikfung."\"";
        $titile_dikfung = " DIKLAT FUNGSIONAL ".$this->siemodel->getAttrdata("a_dikfung","iddikfung",$iddikfung,"dikfung");
    }else{
        $titile_dikfung = " SEMUA DIKLAT FUNGSIONAL ";
    }
    ...
    $q = "select a.nip,CONCAT(a.gdp,IF(LENGTH(a.gdp)>0,' ',''),a.nama,IF(LENGTH(a.gdb)>0,', ',''),a.gdb) AS nama
        ...
        left join r_dikfung k on a.nip = k.nip
        ";
        $rs = $this->db->query("$q $where");
?>
<center>
    <h3>DAFTAR PEGAWAI <?php echo $titile_skpd?></h3>
    <h3>BERDASARKAN <?php echo $titile_kelamin?> <?php echo strtoupper($titile_dikfung)?></h3>
</center><br>
<table border="1">
    <colgroup>
