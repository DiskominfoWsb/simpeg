...
<html>
<head>
    <title>Simpeg Tegal Statistik Jabatan Fungsional Umum</title>
...
<?php
    ...
    $idjabfungum = $this->input->post('idjabfungum');
    $idskpd = $this->input->post('idskpd');

    $where = " where a.idjenkedudupeg not in('99','21') and a.idjenjab = 3 and a.idjabfungum != ''";
    if($idwhere != "1") {
        $where.=" and j.jabfungum = \"".$idjabfungum."\"";
        $titile_jabfungum = " JABATAN FUNGSIONAL UMUM ".$idjabfungum;
    }else{
        $titile_jabfungum = " SEMUA JABATAN FUNGSIONAL UMUM";
    }

    if(($idjenkel != "grand") and ($idjenkel != "sum")) {
    ...
    $q = "select a.nip,CONCAT(a.gdp,IF(LENGTH(a.gdp)>0,' ',''),a.nama,IF(LENGTH(a.gdb)>0,', ',''),a.gdb) AS nama
        ...
        from tb_0823 a force index(index_skpd)
...
<center>
    ...
    <h3>BERDASARKAN <?php echo $titile_kelamin?> <?php echo strtoupper($titile_jabfungum)?></h3>
</center><br>
<table border="1">
    <colgroup>
