...
<html>
<head>
    <title>Simpeg Tegal Statistik Eselon</title>
...
<?php
    ...
    $idesljbt = $this->input->post('idesljbt');
    $idskpd = $this->input->post('idskpd');

    $where = " where a.idjenkedudupeg not in('99','21') ";
    if($idwhere != "1") {
        $where.=" and a.idesljbt = \"".$idesljbt."\"";
        $titile_eselon = " ESELON ".$this->siemodel->getAttrdata("a_esl","idesl",$idesljbt,"esl");
    }else{
        $titile_eselon = " SEMUA ESELON ";
    }
...
<center>
    ...
    <h3>BERDASARKAN <?php echo $titile_kelamin?> <?php echo strtoupper($titile_eselon)?></h3>
</center><br>
<table border="1">
    <colgroup>
