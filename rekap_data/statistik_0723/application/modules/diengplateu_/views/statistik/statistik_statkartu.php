        ...
        <table class="table table-hover table-bordered" width="100%" id="tb-statistik">
            ...
            <tbody>
                ...
                $rs = $this->db->query("SELECT if(a.idjenkel=1,'Pria','Wanita') as kategori, a.idjenkel, SUM(IF(b.nokarpeg!='',1,0)) AS nokarpeg,SUM(IF(b.nokarpeg='',1,0)) AS nonnokarpeg,
                      SUM(IF(b.nokaris!='',1,0)) AS nokaris, SUM(IF(b.nokaris='',1,0)) AS nonnokaris FROM a_jenkel a LEFT JOIN tb_0723 b ON a.idjenkel = b.idjenkel
                      where b.idjenkedudupeg NOT IN (99,21) $where GROUP BY a.idjenkel ORDER BY a.idjenkel");
                ...
                <tr>
                    ...
                    <td><div class="link" align="right" data="0" iskartu="1" skpd="<?=$this->input->post('idskpd')?>" title="Lihat Detail"><?php echo number_format($item->nokarpeg)?></div></td>
                    <td><div class="link" align="right" data="0" iskartu="11" skpd="<?=$this->input->post('idskpd')?>" title="Lihat Detail"><?php echo number_format($item->nonnokarpeg)?></div></td>
                    <td><div class="link" align="right" data="0" iskartu="2" skpd="<?=$this->input->post('idskpd')?>" title="Lihat Detail"><?php echo number_format($item->nokaris)?></div></td>
                    <td><div class="link" align="right" data="0" iskartu="22" skpd="<?=$this->input->post('idskpd')?>" title="Lihat Detail"><?php echo number_format($item->nonnokaris)?></div></td>
                </tr>
                    <?php } ?>
                <tr class="breadcrumb">
                    <td colspan="2">Grand Total</td>
                    <td><div class="link" align="right" data="1" iskartu="1" skpd="<?=$this->input->post('idskpd')?>" title="Lihat Detail"><?php echo number_format(array_sum($sumkarpeg))?></div></td>
                    <td><div class="link" align="right" data="1" iskartu="11" skpd="<?=$this->input->post('idskpd')?>" title="Lihat Detail"><?php echo number_format(array_sum($sumnonkarpeg))?></div></td>
                    <td><div class="link" align="right" data="1" iskartu="2" skpd="<?=$this->input->post('idskpd')?>" title="Lihat Detail"><?php echo number_format(array_sum($sumkarsu))?></div></td>
                    <td><div class="link" align="right" data="1" iskartu="22" skpd="<?=$this->input->post('idskpd')?>" title="Lihat Detail"><?php echo number_format(array_sum($sumnonkarsu))?></div></td>
                </tr>
            </tbody>
        </table>
    </div>
