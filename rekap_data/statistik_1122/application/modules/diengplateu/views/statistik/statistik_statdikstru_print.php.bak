...
<?php
    ...
    $iddikstru = $this->input->post('iddikstru');
    $idskpd = $this->input->post('idskpd');

    $where = " where a.idjenkedudupeg not in('99','21') ";
    if($idwhere != "1") {
        $where.=" and a.iddikstru = \"".$iddikstru."\"";
        $titile_dikstru = " DIKLAT STRUKTURAL ".$this->siemodel->getAttrdata("a_dikstru","iddikstru",$iddikstru,"dikstru");
    }else{
        $titile_dikstru = " SEMUA DIKLAT STRUKTURAL ";
    }
